...
void ui_adauga(UI* ui) {
    ...
    printf("Ziua (1-31): "); scanf("%d", &zi);
    printf("Suma: "); scanf("%d", &suma);
    printf("Tip (1-Intrare, 0-Iesire): "); scanf("%d", &tip);
    printf("Descriere: "); scanf("%s", descriere);

    int eroare = Adauga_Tranzactie(ui->service, zi, suma, tip, descriere);
    if (eroare == 1) printf("Tranzactie adaugata cu succes!\n");
    else printf("Date invalide! Tranzactia nu a fost adaugata.\n");
}
...
void ui_modifica(UI* ui) {
    ...
    printf("ID-ul tranzactiei de modificat: "); scanf("%d", &id);
    printf("Ce doriti sa modificati? (1-Zi, 2-Suma, 3-Tip, 4-Descriere): ");
    scanf("%d", &opt);

    int rez = -1;
    switch (opt) {
        case 1: printf("Zi noua: "); scanf("%d", &val_noua); rez = Modifica_Tranzactie_Day(ui->service, id, val_noua); break;
        case 2: printf("Suma noua: "); scanf("%d", &val_noua); rez = Modifica_Tranzactie_Suma(ui->service, id, val_noua); break;
        case 3: printf("Tip nou (0/1): "); scanf("%d", &val_noua); rez = Modifica_Tranzactie_Tip(ui->service, id, val_noua); break;
        case 4: printf("Descriere noua: "); scanf("%s", desc_noua); rez = Modifica_Tranzactie_Descriere(ui->service, id, desc_noua); break;
        default: printf("Optiune invalida.\n"); return;
    }

    if (rez == 1) printf("Modificare reusita!\n");
    else printf("Eroare la modificare (ID inexistent sau date invalide).\n");
}

void ui_sterge(UI* ui) {
    int id;
    printf("ID-ul tranzactiei de sters: "); scanf("%d", &id);
    if (Sterge_Tranzactie(ui->service, id) == 1) printf("Sters cu succes!\n");
    else printf("ID-ul nu a fost gasit.\n");
}
...
void ui_vizualizare_filtru(UI* ui) {
    ...
    if (opt == 1) {
        ...
        int suma;
        printf("Suma limita: "); scanf("%d", &suma);
...
void Run_Console(UI* ui) {
    ...
    while (running) {
        ...
        switch (optiune) {
            ...
            case 2: ui_modifica(ui); break;
            case 3: ui_sterge(ui); break;
            case 4: ui_vizualizare_filtru(ui); break;
            case 5: ui_vizualizare_ordonata(ui); break;
            case 0: running = 0; printf("La revedere!\n"); break;
            default: printf("Optiune invalida!\n");
        }
    }
}
